...
import { z } from 'zod'
import { type UnresolvedJudgeCardReference, fetchJudgeCardContexts, stripCardReferenceMarkup } from '#/lib/scryfall'
...
function getJudgeResponseText(payload: JudgeBackendResponse) {
  ...
  const citations = Array.isArray(payload.citations)
    ? payload.citations
        .filter((citation): citation is { rule?: unknown; excerpt?: unknown } => !!citation && typeof citation === 'object')
        .map((citation) => {
  ...
  const assumptions = Array.isArray(payload.assumptions)
    ? payload.assumptions
        .filter((assumption): assumption is string => typeof assumption === 'string' && assumption.trim().length > 0)
