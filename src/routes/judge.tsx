...
import { type JudgeAssistantResponse, type JudgeClarification, sendJudgeQuestionToBackend } from '#/server/judge'
    ...
    <main className="mx-auto w-full max-w-5xl px-8 py-8">
      <section className="rounded-2xl border border-zinc-800 bg-zinc-950/70 p-6 shadow-2xl shadow-black/20">
        <div className="border-b border-zinc-800 pb-4">
          ...
          <p className="mt-1 text-sm text-zinc-400">Reference cards with <code>[[Card Name]]</code>.</p>
        </div>
        ...
        {messages.length === 0 ? (
          ...
          messages.map((message) => (
            ...
            >
              ...
              {message.isPending ? <p className="mt-2 text-xs text-zinc-500">Judge model is thinking.</p> : null}
              ...
              {message.role === 'assistant' && message.clarification?.unresolvedCards.length ? (
                <div className="mt-3 border-t border-zinc-800 pt-3">
                  ...
                  <p className="mt-1 text-sm text-zinc-400">{message.clarification.unresolvedCards[0]?.name}</p>
